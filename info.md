...
Point it at one entity, pick a mode, and it produces a bundle of duration, percentage, compliance, and transition sensors for `today`, `yesterday`, `24h`, `7d`, `30d`, `month`, and `year` from a single config-flow pick. A custom Lovelace card renders it as bars, a pie/donut, or a dense multi-frame table.
...
- **Many frames from one pick** — `today`, `yesterday`, `24h`, `7d`, `30d`, `month`, `year`, each toggleable.
